...
- 内容: パスワードのハッシュを PBKDF2-SHA256 210,000 回へ変更した（DECISIONS #019）が、
  security.md §2 の PIN 行は「bcrypt cost 10」のまま据え置いた。P0-08 の task に
  PIN が含まれないため、書かれていない範囲を勝手に変更しないという判断。
  結果として、P0-09 は **PIN のためだけに bcryptjs（純 JS）を導入する**か、
  PIN も PBKDF2 に揃えるかを選ぶことになる。
- 影響: P0-09（PIN ログイン）、`packages/db` の依存関係。
- 暫定対応: なし。判断材料として:
  4 桁 PIN は候補が 10,000 通りしかなく、**ハッシュが漏れた時点でどの方式でも
  総当たりは現実的**（cost 10 でも 10,000 × 87ms ≒ 15 分）。PIN の防御は
  ソルトと 5 回失敗ロックが担っており、KDF の強度差はほとんど効かない。
  一方、bcryptjs を入れると Workers 上で 1 回 100ms 超の純 JS 処理が
  現場系ログインの経路に載る。**P0-09 の着手前に決めること。**
