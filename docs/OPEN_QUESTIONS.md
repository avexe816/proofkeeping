...
- 暫定対応: 受け口を作らなかった。写真の受け取りは
  リサイズ済みバイナリ・EXIF 除去・R2 への書き込みを伴い、
  既存の `lib/photo/upload.ts`（P1-11）と `inspectionUpload.ts`（P2-06）が
  **すでに 2 通りの書き方に分かれている。** 3 つ目を増やす前に共通化したい。
- 決める人: 3 経路を 1 つの受け口へ寄せるか、経路ごとに書くか。
  M-13（P2-13）は写真撮影まで 3 タップで到達することが完了条件なので、
  **P2-13 の前に決める必要がある。**
