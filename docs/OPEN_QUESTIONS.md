...
- 暫定対応: リポジトリ層は組織で絞るだけにし、施設の絞り込みを
  意図的に掛けていないことを `NO_PROPERTY_SCOPE` という明示の引数で
  ソースに残した（省略できない形にしてある）。
  **到達可否そのものは P0-10 の `assertPermission()` が判定する前提。**
  P1-14 の人員配分画面は担当施設のスタッフだけを出す必要があるはずで、
  その時点で `property_assignment` 経由の絞り込みが要るなら
  リポジトリ側に追加する。**P0-10 の着手前に判断が要る。**
