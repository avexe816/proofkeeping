...
- [x] P5-02 取引先マスタ — 締め日・支払サイト・端数処理・請求先メールと CC。
      **削除の口が無い**（`isActive = false`）。コードは付け替え不可
      （DECISIONS #122）
- [x] P5-03 料金設定 — 5 段階の解決と有効期間を `packages/billing` の
      純粋関数に。**更新の口が無い**（値上げは行の追加。終了は `close` /
      DECISIONS #123）。該当が無ければ `null`（0 を返さない）
- [x] P5-04 集計と料金計算エンジン — 税率ごとに 1 回だけ端数処理。
      **単価未設定は ¥0 明細＋警告**（黙って除外しない）。赤伝が元の伝票と
      打ち消し合うよう、丸めは絶対値で行って符号を戻す
- [ ] P5-05 月次締めと集計バッチ
