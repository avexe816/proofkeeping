...
最終更新: 2026-08-13（P2-16 完了。一括承認の削除と検査方式の移行）

## 現在のセッション

```
task: P2-16（P1 暫定機能の移行・削除）
状態: 完了。**一括承認は代替を作らずに消した。** 検査方式は移行済み。
      旧列 `property.inspectionRequired` は**残っている**（3 段階の②）。
```

**消したもの（§13.1）。**
`packages/engine` の `RoomStatusTrigger` から `bulkApprove`、
監査ログから `task.bulkApproved`。**P1 に到達経路は無かった**（API も画面も
作られていない）ので、既存の監査ログにこの値は 1 件も無い。だから
「一度使った文字列を変えない」の例外にせず消せた。
`housekeepingStatusFor("bulkApprove", …)` が `undefined` を返すことを見る
テストを添えてある。ここが緑でなくなったら一括承認が戻っている。

**移行したもの（§13.2 / OPEN_QUESTIONS #044 → 回答済み）。**
`0011_p2_16_inspection_policy_backfill.sql` が全施設に検査方式の行を作る。
仕様の SQL は `gen_random_cuid()` を使う PostgreSQL 前提なので、
**ID は施設 ID の ULID 部分を流用した**（`…__prop_01J…` → `…__ipol_01J…`）。
1 施設 1 行で衝突せず、決定的なので `WHERE NOT EXISTS` と合わせて冪等
（3 回流して件数が変わらないことを実 SQLite で確認した）。
**既に行がある施設は触らない。** W-02 で設定済みの値が正。
`createProperty()` と `seed()` も施設と検査方式を一組で書くようにした
（行が無い施設が生まれ続けると移行が終わらない）。

**旧列を消していないのは意図**（architecture.md §6 の②）。
読み取り側の `policyFromLegacyFlag()` も残してある。移行が届いていない
シャードで既定の `ALL` に落ちると、**全タスクが検査待ちで滞留する。**
**次リリースで旧列の DROP とこの関数の削除を行うこと（③）。**

**残存タスク（§13.3）。**
`POST /api/v1/tasks/:taskId/inspection/override`（1 件ずつ・理由必須・
監査ログ）と `GET /api/v1/inspections/stranded`（業務日で絞らない）。
**まとめて閉じる口を作っていない。** 作ると一括承認が名前を変えて戻る
（DECISIONS #090）。上書きは `inspectionSkipped` と `EMERGENCY_OVERRIDE`
だけを書き、`inspectionResult` を書かない。§10.1 の分母は「検査の記録が
あるタスク」なので、上書きした分は検査の母数に入らない。
権限は新設の `inspection.emergencyOverride`（施設責任者以上。
**`INSPECTOR` は DENY**）。**画面は作っていない**（OPEN_QUESTIONS #057）。
移行のときにしか使わない操作を管理画面に常設すると、
「検査せずに閉じる」ボタンが残り続ける。

次: **P2-17 現場検証（人間が実施）★出荷判定。**
    自社施設で 2 週間・紙を全廃できるかを見る。ここは自分では実施できない。
    P2 の実装 task は P2-16 で全部終わっている。
