...
最終更新: 2026-08-15（P6-09 完了。**P6-06 は人間待ち**）

## 現在のセッション

```
task: P6-09（通知基盤 IN_APP → EMAIL）
状態: 完了。112 task。
```

**`IN_APP` に配信の実体を作らなかった**（DECISIONS #146）。§5.1 の 10 イベント
のうち 5 つが `IN_APP` を既定チャネルに持つのに、**§2 のデータモデルに通知を
貯める表が無い。** §5.2 MUST の「必ず画面内でも同じ情報を提示する」と合わせて
読むと、`IN_APP` は M-02 の再清掃バッジ・M-08 検査待ち・客室ボードといった
**既にある画面**を指す。表と通知センターを足すと、「通知センターを見ないと
分からない」導線が生まれて MUST に逆行する。

**`CLEANER` の境界を二重に締めた**（DECISIONS #147）。§5.1 の表の `audience` と
`CLEANER_ALLOWED_EVENT` の定数の両方を見る。表だけで守ると、**誰かが
`audience` に 1 語足した瞬間に清掃スタッフへ通知が流れ始める。**

**重複は `CONFIG` KV の `dedupeKey` で止める**（DECISIONS #148）。D1 を引く前に
見て、送り終えてから置く。先に置くと、途中で落ちたときに 1 通も送らないまま
「送った」ことになる。

**業務通知を `document_delivery` に記録しない**（DECISIONS #149）。あれは電子
取引の記録そのもの（billing.md §2）で、混ぜると「いつ請求書を送ったか」を
引くのに除外条件が要るようになる。

繋いだ producer は 2 つ（`integration.error` / `finding.high`）。残り 8 つは
それぞれの業務フロー側が `notify()` を 1 回呼べば動く。**task の完了条件は
「10 イベントが定義されている」**なので、基盤としてはここで閉じている。
