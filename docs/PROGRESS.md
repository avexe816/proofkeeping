...
最終更新: 2026-08-12（P1-07〜P1-13 完了。**清掃員モバイル 4 画面が動く**）

## 現在のセッション

```
task: P1-07 M-01 PIN ログイン / P1-08 M-02 本日のタスク /
      P1-09 M-03 タスク詳細 / P1-10 M-04 チェックリスト /
      P1-11 写真アップロード / P1-12 オフラインキュー / P1-13 ホーム画面追加バナー
状態: 7 task 完了（Batch 2 = 現場モバイル）。**「開始 → 記録 → 完了」が
      オフラインでも通る。** PC 管理画面（W-03/04/05/16/17）は未着手。

      P1-07: `/m/login`。3 項目は orgShortId + スタッフ番号 + PIN。
             画面内テンキー（OS キーボードを出さない）。レート制限は API と
             同じバケツ。**`pinMustChange` は見ていない**（#037）。
      P1-08: `/m/today`。並び順・5 段カウンタは `packages/engine` の
             `taskBoard.ts`（node でテストできる側）。「開始する」は 1 タップ。
             30 秒自動更新 + 手動 + プルダウン。
      P1-09: `/m/task/:id`。経過は時間ログから作る（中断中は増えない）。
             超過はオレンジ→グレー。**赤に相当する値を返さない**（INV-05）。
      P1-10: `/m/task/:id/checklist`。3 値・1 項目ずつ・完了セクションは自動折りたたみ。
             **`POST /tasks/:id/checklist` が `/:taskId/:action` に吸われて
             404 になっていたのを直した**（Hono は登録順に照合する）。
      P1-11: `POST /api/v1/tasks/:id/photos`（multipart）。撮影は
             `input[capture]`。EXIF はクライアント（canvas 再エンコード）と
             サーバー（セグメント除去）の両方で落とす。`clientId` で冪等。
      P1-12: `lib/offline/`。判断（policy）と IndexedDB / fetch（queue）を分離。
             Background Sync は使わない。5 トリガー・直列・指数バックオフ。
      P1-13: iOS Safari のタブでのみ案内。30 日間は再表示しない。
次: **P1-14 W-04 タスク管理・人員配分から。** PC 管理画面（Batch 3）へ進む。
    下の申し送り 1〜8 を読むこと。

--- P1-07〜P1-13 からの申し送り ---
申し送り 1: **`apps/web/tsconfig.json` の `lib` に DOM を足した。**
            `/m/*` は IndexedDB・canvas・visibilitychange を使う。副作用として
            **Worker 側のコードで `document` を触ってもコンパイルが通る**
            ようになった（実行時に落ちる）。ブラウザ専用 API を
            loader / API ハンドラへ持ち込まないこと。
            `lib/auth/pbkdf2.ts` の `salt` に 1 行の注記が入っている。
申し送り 2: **`/api/v1/tasks` の登録順が意味を持つ。** `/:taskId/:action` は
            必ず最後に置くこと。Hono は登録順に照合し、静的な区間を
            優先しない。先に置くと `/:taskId/photos` も `/:taskId/checklist` も
            吸われて 404 になる。`tasks.spec.ts` に回帰テストがある。
申し送り 3: **モバイルの英語カタログを増やしていない。** `m.*` のキーは
            `ja.json` にだけある（欠けたキーは `ja` に落ちる）。
            英語対応は P1-18 の担当。翻訳が揃った言語から `LOCALES` に足すこと。
申し送り 4: **下部タブバーを置いていない。** プロトタイプ（pk-02）は
            タスク / 検査 / 実績 / 設定の 4 タブを描くが、検査は P2、
            実績は P1-17、設定は P1-18 で、押しても何も起きない。
            画面が揃った task が `routes/m/layout.tsx` へ足すこと。
申し送り 5: **一覧（M-02）の経過は「開始時刻からの時計」**で、中断を
            差し引いていない。実作業時間は時間ログからしか出せず、一覧で
            引くとタスクの数だけクエリが増える（§13）。M-03 は正確な値を出す。
            一覧にも正確な値が要るなら、業務日でまとめて集計する関数を足すこと
            （タスクごとに引く実装にしない）。
申し送り 6: **写真の削除 API を作っていない。** P1 の task に無く、
            INV-27 と同じ向きに倒してある。必要になった task が
            監査ログとセットで足すこと。
申し送り 7: **メモだけを保存する API が無い。** M-03 のメモは中断・完了の
            `note` に添えて送っている。メモ単独の保存が要るなら口を足すこと。
申し送り 8: **機内モードの通し操作（P1-12 の完了条件 1 件）は未検証。**
            自動テストは規則の側だけを固定してある。実機テスト（P1-19）で
            確かめ、P1-12 のチェックを消すこと。

--- P1-01〜P1-06 からの申し送り（引き続き有効）---
申し送り 1: **`room.housekeepingStatus` を作っていない**（OPEN_QUESTIONS #034）。
            §11.1 の客室ステータス同期は P1-16 の担当で、P1-01 の やること
            8 表に含まれない。P1-16 が列を足し、`applyTransition()` の
            呼び出し側で同期させること。列の追加は後方互換なので順序の問題は無い。
...
- [x] P1-01 スキーマ: 清掃タスク
- [x] P1-02 標準時間マスタと設定画面 — **API のみ。画面（W-17）は未着手**
- [x] P1-03 タスク自動生成
- [x] P1-04 W-05 当日の客室状況入力 — **API のみ。画面（W-05）は未着手**
- [x] P1-05 ステータス遷移 API
- [x] P1-06 チェックリスト定義 — **API のみ。画面（W-16）は未着手**
