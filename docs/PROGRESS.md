...
最終更新: 2026-08-12（P1-21 完了。**複数施設を担当する清掃員の 1 本のリストができた**）

## 現在のセッション

```
task: P1-21 施設グループ表示
状態: 1 task 完了（Batch 4 の 1 本目）。**M-02 が複数施設に対応した。**

      P1-21: `dailyRoute` テーブル（migration 0005）と
             `GET /api/v1/tasks/my-day`（1 リクエストで全施設分 / §19.6 MUST）。
             M-02 は濃色ヘッダ帯の施設グループ＋移動ブロック＋
             全施設をまたぐフィルタ。オフラインは `my-day` の応答を
             **1 日単位**で IndexedDB へ保存し、取得時刻を明示する。
             §19.8 の「施設が変わる開始の確認」を**先に入れた**（#041）。
             `dailyRoute` は**読み取りだけ**。P1 の間この表は常に空で、
             移動ブロックは現場に出ない（#037）。
次: **P1-22 施設選択画面**（§19.4）と **P1-23 施設検証**（§19.8 / §19.9）。
    どちらも P1-21 に依存し、依存は満たされた。P1-23 は
    完了条件 7 件のうち 2 件を P1-21 が済ませてある（#041）。

    **P1-19 実機テスト / P1-20 現場検証は人間が実施。** 下を読むこと。
    P1-21 の task ファイルは「P1-08 の単一施設版を現場で確認してから
    着手する」を前提に挙げているが、**その確認は未実施のまま着手した。**
    前セッションの申し送り（P1-21〜23 は P1-19 / P1-20 に依存しない）に
    従ったもので、判断は人間に確認してほしい。
```
