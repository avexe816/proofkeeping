...
状態: 完了。W-25 `/app/settings/room-types` と `/api/v1/room-types`。
      ...
      **着手の前提が動いている。** P1-19 実機テスト / P1-20 現場検証は
      「完了とみなす」という人間の判断で `[x]` にした。**実機確認と
      2 週間の現場運用そのものは行われていない。** P1-24 の task ファイルは
      「着手は P1-20 の判定後」と書いており、そこを人間が解除した形。
      **P2 の着手条件（CLAUDE.md §9）もこの判断に乗っている。**
...
--- 人間にお願いすること ---
1. **P1-19 / P1-20 を本当に実施するかを決めること。** 今回は「完了とみなす」
   という指示で `[x]` にしたが、実機テスト（testing.md §6）と自社施設 2 週間の
   紙全廃運用は行われていない。**P2 以降はこの判断の上に積み上がる。**
   実施しないまま進めるなら、CLAUDE.md §9 の P2 着手条件を書き換えるか、
   「判定を省略した」ことを残しておくのが筋。
2. **P0-02 Cloudflare リソース作成**（未着手のまま）。実 D1 が 1 本しか無く、
...
- [x] P1-17 M-11 自分の実績
- [x] P1-18 多言語（英語）
- [x] P1-19 実機テスト（人間が実施）— **人間の判断で完了とみなした**（2026-08-12）。
      testing.md §6 の実機確認そのものは行われていない。
- [x] P1-20 現場検証（人間が実施）★出荷判定 — **人間の判断で完了とみなした**（2026-08-12）。
      自社施設 2 週間の紙全廃運用そのものは行われていない。**P2 の着手条件は
      この判断に依存している**（CLAUDE.md §9）。
- [x] P1-21 施設グループ表示
