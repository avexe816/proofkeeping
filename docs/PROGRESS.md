...
最終更新: 2026-08-15（P7-15 / P7-16 / P7-01 は Step 5 の土台まで）

## 現在のセッション

```
task: P7-15 / P7-16 / P7-01
状態: P7-15 完了。P7-16 は文書まで。P7-01 は Step 5 の土台まで。123 task。
```
...
残り: 会社種別の列（Step 1）/ ウィザードの進行状態 / 6 ステップの画面 /
`SEED_CHECKLIST_TEMPLATES` を seed 専用の置き場所から出す。
...
| task | 誰が | 何をする |
|---|---|---|
| **P7-01** | **Claude** | **Step 5 の土台まで実装済み。** 残りは会社種別の列 / 進行状態 / 6 ステップの画面 |
...
- [ ] P7-01 セットアップウィザード — **Step 5 の土台まで実装**
      ...
      **3 表を 1 関数で作る**（#178）。管理者のメール招待は未実装（#101）。
      残り: 会社種別の列 / ウィザードの進行状態 / 6 ステップの画面
- [ ] P7-02 ログイン案内カード PDF
- [ ] P7-03 トライアル管理
- [ ] P7-04 Stripe 連携
- [ ] P7-05 解約とエクスポート
