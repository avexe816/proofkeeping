...
最終更新: 2026-08-13（P3-09 / P3-10 / P3-12 完了。P3 の実装 task が全部終わった）

## 現在のセッション

```
task: P3-09 / P3-10 / P3-12（Batch 3: ベースラインとデータ品質）
状態: 完了。**P3 の実装 task はこれで全部。** 残るのは P3-13（人間が実施）。
      週次バッチが `consumption_baseline` を書き、W-21 が確認・上書き、
      W-22 が入力品質を出す。**判定は 1 つも無い**（§0.2）。
      閾値との突き合わせは P4。
```

**週次バッチ（§5.1）。** cron `0 18 * * 6`（土曜 18:00 UTC = 日曜 03:00 JST）
→ 全アクティブ施設を `QUEUE_BASELINE_LEARNING` へ → コンシューマが
90 日ぶんの観察を読んで統計量を書く。**ウィンドウ終端は前の業務日**
（DECISIONS #103。03:00 は日締め前で、その日はまだ終わっていない）。

**観察 → サンプルの平坦化を engine に切り出した**（DECISIONS #101）。
`roomObservation` は 1 タスク 1 行、`linenRecord` は品目ごとに 1 行で、
同じ品目が複数の経路から来る。**列 > JSON > リネン**の優先順位で
1 タスク × 1 品目 = 1 件にする。二重に積むと分布が壊れる。

**手動上書きは週次バッチで消えない**（§5.5 MUST / DECISIONS #104）。
今回の集計に出なくなった組み合わせも、上書きがあれば残す。

**外れ値除外率の分母は観察の件数**（DECISIONS #102）。品目ごとの行数で
割ると、施設設定（`enabledItemCodes`）の品目数で率が変わり、施設どうしを
比べられなくなる。

**越境テストの `UNCOVERED_TABLES` から観察系の 2 表が消えた**
（`tests/tenant-isolation/baseline.spec.ts`）。P3 で足した 6 表は全部
カバー済み。

次: **P3-13（人間が実施）。** 4 週間の蓄積と、入力率 95% / 既定値率 90% 未満 /
    主要な組み合わせの 80% が `isReliable` を待つ。W-22 がこの 3 つを
    そのまま出す。**通過するまで P4 に着手しない**（CLAUDE.md §9）。

**P4 着手前に決めること。** OPEN_QUESTIONS #058（除外ルール①の適用範囲）、
#061（`cupsUsed` / `extraFutonUsed` の品目コード）、#062（施設ごとの集計日数）。
