...
最終更新: 2026-08-12（P1-01〜P1-06 完了。**Phase 1 のバックエンド基盤が揃った**）

## 現在のセッション

```
task: P1-01 スキーマ / P1-02 標準時間 / P1-03 タスク自動生成 /
      P1-04 当日の客室状況 / P1-05 ステータス遷移 / P1-06 チェックリスト定義
状態: 6 task 完了（**バックエンドのみ**）。画面は 1 本も作っていない。
      W-05 / W-16 / W-17 と M-01〜M-04 は P1-07 以降の担当。

      P1-01: 8 表を追加（cleaningTask / taskTimeLog / taskPhoto / standardTime /
             dailyRoomPlan / checklistTemplate / checklistItem /
             taskChecklistResult）。**enum の語彙は PK-SPEC-P1 §2.1 を採り、
             チェックリストの実施結果だけ実装契約書の 3 値**（INV-22）。
             食い違いは OPEN_QUESTIONS #032 / DECISIONS #031。
             `property.inspectionRequired` を 1 列足した（§5.2 が名指しする列）。
      P1-02: standardTime マスタ + GET/PUT /api/v1/standard-times。
             既定分数（§3.1）より設定が優先。設定が無くても動く。
      P1-03: **Cron Trigger を 1 本追加**（`0 17 * * *` = 02:00 JST）。
             差分生成は `packages/engine` の純粋関数。冪等性は
             ①計画から着手済みを外す ②UPDATE の条件に status ③一意制約
             の 3 段構え。手動再生成は POST /api/v1/tasks/generate。
      P1-04: CSV 取込・一括入力・**「全室アウト清掃として生成」**（§3.4 MUST）。
             CSV は 1 行の誤りで全体を落とさず、読めなかった行番号を返す。
      P1-05: 8 操作（§5.3 の 6 + resume + unblock）。`Idempotency-Key` は
             `taskTimeLog` の一意列で持つ（DECISIONS #034）。
             actualMinutes は時間ログから毎回作り直す。
      P1-06: 3 階層の継承（組織→施設→客室タイプ）。既定テンプレート 2 種を
             seed。`templateVersion` は展開時に固定し、以後更新しない。
次: **P1-07 M-01 PIN ログイン画面から。** Batch 2（画面）へ進む前に、
    下の申し送り 1〜7 を読むこと。
