...
最終更新: 2026-08-12（P1-02 / P1-04 / P1-06 / P0-22 の未達分を実装。
**§10.1 の PC 5 画面が全部そろった**）

## 現在のセッション

```
task: P1-02 / P1-04 / P1-06 / P0-22 の「未達（意図的）」を閉じる
状態: 4 task ぶんの未達を実装（新しい task ファイルは作っていない / #048）。

      **なぜ既存 task なのか。** P1 の未チェックは P1-19 / P1-20 の 2 件で、
      どちらも人間が実施。P2 以降は P1-20 の出荷判定まで着手禁止
      （CLAUDE.md §9）。一方 `[x]` の task に「未達（意図的）」として
      残された作業が 4 件あり、そちらは人間の作業を待たずに進められる。

      W-17 `/app/settings/standard-times`（P1-02）
           客室タイプ × 清掃種別。**未設定のセルは既定分数を初期値として
           表示**し「既定値」の印を付ける（#050）。保存は触られたセルだけ。
      W-05 `/app/p/{id}/plan`（P1-04）
           §10.3 の 7 列＋入力元。**「全室アウト清掃として生成」**は
           §3.4 MUST の逃げ道。既存入力を上書きするので確認を 1 段挟む。
      W-16 `/app/settings/checklists`（P1-06）
           3 階層を浅い順に並べ、**その施設で実際に効くものに印**を付ける
           （生成と同じ `resolveTemplate()` を通す）。項目は 1 行 1 項目の
           テキストで編集（#051）。保存で版が 1 上がることを画面に出す。
      §24.5（P0-22）
           客室の無効化で未完了タスクの件数を提示して確認を求める。
           **タスクは取消さない**（#052）。

      あわせて `/app/settings/*` の 4 画面をサイドバーへ載せた（#053）。
      客室マスタ（P0-22）と事業者税務（P0-16）は**ルートが実在するのに
      サイドバーに無く、URL を直に打つしか到達できなかった。**

次: **P1-19 実機テスト / P1-20 現場検証。どちらも人間が実施。** 下を読むこと。
    ここを通すまで P2 に着手しない（CLAUDE.md §9）。
    **自動で進められる P1 の作業はこれで無い。**
```

--- 人間にお願いすること（P1-19 / P1-20）---
P1-19 実機テスト（testing.md §6）:
  iPhone SE / iPhone 14 Safari / Android Chrome で `/m/*` の全操作、
  手袋でのタップ、屋外での視認、**機内モードでの通し操作 → 復帰後の自動送信**。
  最後の 1 件は P1-12 の完了条件のうち唯一未検証のもの。確認できたら
  P1-12 の行から注記を消すこと。
  **P1-21 で M-02 が作り替わったので、M-02 は必ず再確認すること。**
P1-20 現場検証（★出荷判定）:
  自社施設で 2 週間、紙を全廃して運用する。通過するまで P2 に着手しない
  （CLAUDE.md §9）。
  **今回 W-05 / W-16 / W-17 が入ったので、管理側の 1 日の流れ
  （客室状況の入力 → タスク生成 → 配分 → 盤面）を通して見られる。**
  紙を全廃する運用に必要な画面はこれで揃っている。
...
申し送り 2: **`roomType` を書く経路が無い。** `listRoomTypes()` を足したが、
            客室タイプを登録・編集する画面も API も無い。seed が入れた
            ものを読むだけ。**W-05 のタイプ列も W-17 の行も、seed 以外の
            環境では空になる。** → **P1-24 として起票した**（人間の指示）。
            `nav.propertySettings` は PLANNED のまま。
...
- [ ] P1-24 施設設定：客室タイプ管理 — `roomType` の登録・編集画面と API。
      **読む関数しか無く、書く経路が一つも無い。** seed の無い環境では
      W-05 の客室タイプ列が空になり、W-17 は表が 0 行になる。
      着手は **P1-20 の出荷判定を通過してから**（検証中に画面を足すと
      検証対象が動く）。P2 より先に置くのは、P2 の検査ポリシーが
      客室タイプ別を持ちうるため。
