...
最終更新: 2026-08-14（P6-05 / P6-07 / P6-08 完了。**P6-06 は人間待ち**）
...
§11 の未決事項 1 が開いたまま（workflow.md §2 の「外部 API の接続情報が
必要 → 飛ばして次へ」）。§3.2 MUST が「想定で作らない」と定めるため、
汎用アダプタでの代替もしない。

**P6-07 は P6-06 を待たずに実装した**（DECISIONS #141）。§3.4 の 2 つの判断
（5/15/60 分のリトライと 5 回でのブレーカー）は、見ているのが
`consecutiveFailures` と失敗回数だけで、**アダプタを 1 つも参照しない。**
依存をそのまま読むと P6-07 も一緒に止まるが、止める理由が無かった。

**「ERROR でも照合が完走する」を依存の不在で押さえた。** 照合バッチと
CSV 取込が `integration` を 1 度も読まないことを spec で検査している。
振る舞いのテストは「たまたま今は通る」を通してしまう。

**自動マッピングは完全一致だけ**（DECISIONS #142）。§7.2 の見本そのものが
`305 ←→ 0305` を「手動設定」と描いている。外したときに起きるのは
「302 号室の稼働記録が 303 号室に入る」ことで、誤りの向きが悪すぎる。

**`actorType` 不明を数に入れる**（§4.3 / P6-08）。数えない実装だと、鍵の
種別を返さない機種で R002 / R013 が一度も立たない。§4.3 が減点（−25）を
定めているのは「不明でも立てたうえで弱く出す」ことを求めているため。
**不明を `GUEST_KEY` に書き換えてはいない。**

次: **P6-09（通知基盤 IN_APP → EMAIL）。**
