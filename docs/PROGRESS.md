...
最終更新: 2026-08-12（P1-14〜P1-18 完了。**PC 管理画面と客室ステータスが繋がった**）

## 現在のセッション

```
task: P1-14 W-04 タスク管理・人員配分 / P1-15 W-03 客室ボード・M-10 /
      P1-16 客室ステータス同期 / P1-17 M-11 自分の実績 / P1-18 多言語（英語）
状態: 5 task 完了（Batch 3 = PC 管理画面と客室ステータス）。
      **タスクの状態が客室の状態へ伝わり、盤面と配分が動く。**

      P1-14: `/app/p/:id/tasks`。自動配分は**必ずプレビューを挟む**（§4.1 MUST）。
             作業中の担当変更は見送って警告を出し、確認して送り直す（§4.2）。
             ドラッグ＆ドロップと一括選択の**両方**を置いた。
             §4.1 の「担当フロアの希望順」はデータが無く、スタッフ番号順に
             した（#039）。
      P1-15: `/app/p/:id/board`（W-03）と `/m/board`（M-10）。**盤面は
             `packages/engine` + `ui/RoomBoard.tsx` で共有。** 入室不可は
             青（契約 §11.2）。タップで担当者・経過時間・写真枚数。
      P1-16: `room.housekeepingStatus` を追加（#034 を解消）。同期の表は
             `packages/engine/src/roomStatus.ts`。手動上書きは理由必須で
             `room.statusOverridden` を残す（**P4 の R010 が使う**）。
             `unblock` の行が §11.1 に無く、`DIRTY` へ戻す判断をした（#038）。
      P1-17: `/m/me`。**平均だけ**を 20 件の閾値で伏せる（#035 / #040）。
             件数と合計作業時間は事実なので常に出す。
      P1-18: `en.json` にモバイル全画面。言語切替は M-11 の中（§12.3）。
             チェックリストの未翻訳に「日本語のみ」。
次: **P1-19 実機テスト（人間が実施）。** 下の「人間にお願いすること」を読むこと。
    P1-21〜P1-23（複数施設）は P1-19 / P1-20 に依存しないので、
    実機テストと並行して進められる。
...
申し送り 1: **`taskPhoto` が `repositories.spec.ts` の検証表に無い。**
            `REPOSITORY_MODULES` に `taskPhoto` が登録されておらず、
            **P1-11 が足した 6 関数と P1-15 が足した `countPhotosByTask()` が
            組織条件の自動検査を受けていない。** 実装側は
            `withTenantScope()` を通しているが、検査は掛かっていない。
            次に `packages/db` を触る task が登録すること。
