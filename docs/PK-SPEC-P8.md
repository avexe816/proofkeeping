...
**保存期間（P8-11 / オーナー判断 2026-08-22。境界と原子性は同日の hotfix / DECISIONS #271）**
...
| v1.3 | 2026-08-22 | §1.4 の保存期間を hotfix（DECISIONS #271）。削除の境界を**満了日の翌日以降**へ（民法 140 条・143 条 2 項）。**暦として存在しない退職日は削除対象にしない**。**DELETE と監査ログを同じ D1 `batch()` で原子的に**実行し、監査ログの件数は**DB に実在した行数**（候補数ではない）とする旨を明記 |
