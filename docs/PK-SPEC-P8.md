# ProofKeeping 製品仕様書
## PK-SPEC-P8 — Phase 8「Workforce と Inventory」 v1.2
...
- 削除の実行を `residency.deleted` として監査ログに残す。**0 件でも残す。**
  **監査ログに氏名・種別・期限・更新申請日・スタッフ ID を載せない**（残すのは件数のみ）。
...
| v1.2 | 2026-08-22 | §1.4 に**保存期間**を追加。「在留資格の記録は、従業員の退職日から 3 年が経過した時点で物理削除する」（オーナー判断 / P8-11 / DECISIONS #264）。起算・除外条件・監査ログに値を載せない旨・設定不可を明記 |
