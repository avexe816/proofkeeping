ProofKeeping 製品仕様書
PK-SPEC-P0 — Phase 0「基盤構築」 v1.2 改訂版
文書ID: PK-SPEC-P0
バージョン: v1.2（v1.0 からの破壊的変更を含む）
発行日: 2026-08-10
前版: v1.0（2026-08-10）
...
> **貼り付け先**: `docs/PK-SPEC-P0.md` の末尾（§22 の後）
> **同時に修正**: 文書冒頭のバージョン表記を `v1.1` → `v1.2` に変更
> **前提**: v1.1（Cloudflare 構成・16 シャード）の内容はそのまま有効

---

## 23. 施設セレクタとマルチ施設 UI（新設）

### 23.1 基本方針

1 アカウントで複数施設を扱う。ロールによって「切替できる範囲」と「既定で見る画面」が異なる。

| ロール | 切替範囲 | 既定画面 | 全社ビュー |
|---|---|---|---|
| `OWNER` | 組織の全施設 | 全社サマリー | あり |
| `ORG_ADMIN` | 組織の全施設 | 全社サマリー | あり |
| `AUDITOR` | 組織の全施設（読取） | 全社サマリー | あり |
| `PROPERTY_MANAGER` | `allowedPropertyIds` のみ | 担当施設の客室ボード | **なし** |
| `INSPECTOR` | 担当施設のみ | 検査待ち一覧 | なし |
| `CLEANER` | 担当施設のみ | 本日のタスク | なし |
| `VENDOR_ADMIN` | 受託施設のみ | 全受託施設ビュー | あり（受託範囲内） |

**MUST**: 全社ビューを持たないロールに「全社サマリー」の項目を表示しない。UI 上で非表示にするだけでなく、API も 403 を返す。

### 23.2 PC 上部バーの構成

```
┌────────────────────────────────────────────────────────┐
│ ProofKeeping  [🏨 サンプルホテル東京  60室 ▾]   🔔2  田中▾ │
└────────────────────────────────────────────────────────┘
                      ↓ クリックで展開
        ┌──────────────────────────────────────┐
        │ サンプル運営株式会社 · 3施設            │
...
**MUST**:
- セレクタは全画面の上部に常設する。画面ごとに位置を変えない。
- ドロップダウン内に各施設の状態サマリーをミニバッジで表示する。順序は `清掃済 / 作業中 / 未清掃`。要確認項目がある施設は `要確認` バッジを追加する。
- 現在選択中の施設に `✓` を表示する。
- 施設数が 8 を超える場合、ドロップダウン上部に検索入力を出す。
- 「施設を追加・編集する」は `ORG_ADMIN` 以上にのみ表示する。

### 23.3 状態サマリーの取得

ドロップダウンを開くたびに全施設分のタスクを集計してはならない。

**MUST**: `dailyPropertyRollup`（v1.1 §19.6）から取得する。1 リクエストで組織内の全施設分を返す専用エンドポイントを用意する。
...
{
  "data": [
    { "propertyId":"o7k2m9__prop_A", "code":"HTLA", "name":"サンプルホテル東京",
      "roomCount":60, "ready":48, "inProgress":6, "dirty":4, "openFindings":0 },
    { "propertyId":"o7k2m9__prop_B", "code":"INOS", "name":"サンプルイン大阪",
      "roomCount":40, "ready":31, "inProgress":5, "dirty":4, "openFindings":2 }
  ]
}
```

- キャッシュは 60 秒。KV に保持する。
- ロールに応じて返す施設を絞る。クライアント側でフィルタしない。

### 23.4 選択状態の保持

**MUST**:
- 選択した施設をセッション（KV）に保存する。リロード・タブを開き直しても維持する。
- `POST /api/v1/auth/switch-property` の引数に `propertyId` または `"ALL"` を受け付ける。
- `"ALL"` を指定できるのは全社ビューを持つロールのみ。それ以外は 403。
- 切替時に `allowedPropertyIds` を再検証する。セッションに保存された施設が権限から外れていた場合、既定施設へ戻す。
- 施設切替を `AuditLog` に記録しない（頻度が高く、ノイズになる）。ただし `"ALL"` への切替は記録する。

### 23.5 URL 設計

施設は URL に含める。ブックマークと共有を可能にするため。

```
/app/p/{propertyId}/board       単一施設
/app/org/dashboard              全社サマリー
```

**MUST**: URL の `propertyId` とセッションの選択施設が異なる場合、URL を正とし、セッションを更新する。ただし権限外の `propertyId` なら 404 を返す（v1.1 §19.4 第 2 層）。
...
**方式A（自主維護）を全顧客の既定とし、方式B（PMS連携）はオプションとする。**

理由: 10〜80 室の中小施設は API を持たない PMS を使っていることが多い。「PMS を連携しないと使えない」設計にすると導入自体が成立しない。

| | 🅰 自主維護 | 🅱 PMS連携 |
|---|---|---|
| 実装フェーズ | **P0** | P6 |
| 外部依存 | なし | PMS の API が必要 |
| 登録方法 | 範囲一括 / CSV / 個別 | PMS から取得 → 差分承認 |
| 自動上書き | — | **しない** |
| 清掃専用の部屋 | 自由に追加可 | ProofKeeping 側でのみ管理 |
| 料金 | 標準機能 | ¥2,000〜5,000 / 施設・月 |
...
**MUST**:
- 100 室を 3 分以内に登録できること（P7-01 の受け入れ基準と一致）。
- 範囲指定で欠番を除外できること（例: `301-320` から `304, 314` を除く）。日本の宿泊施設では 4 を避ける慣習がある。
- 既に存在する部屋番号はスキップし、スキップ件数を表示する。エラーにしない。
- CSV 取込にも対応する。
...
PMS に存在しないが清掃対象となる場所を登録できる。

```
パントリー / 備品庫 / 従業員休憩室 / 共用トイレ / 大浴場 / ロビー
```

**MUST**:
- `Room` に `isSellable` フラグを追加する。既定 `true`。
- `isSellable = false` の部屋は客室ボードに別セクションで表示し、客室数の集計に含めない。
- 稼働照合（P4）の対象外とする。稼働記録が存在しないのは正常なため。

```ts
// Room スキーマへの追加
isSellable: integer("is_sellable", { mode: "boolean" }).notNull().default(true),
sourceType: text("source_type").notNull().default("MANUAL"), // MANUAL | PMS_SYNC | CSV
externalRoomId: text("external_room_id"),  // PMS 側の ID（方式B用）
```

### 24.4 方式B — 差分承認方式（MUST）

**PMS から取得した客室マスタで既存データを自動上書きしてはならない。**

```
1. PMS から客室一覧を取得
2. externalMapping で照合
3. 差分を 3 分類して提示
     追加: PMS にあり、ProofKeeping にない
     削除: ProofKeeping にあり、PMS にない
     変更: 客室タイプ・定員・フロアが異なる
4. 管理者が個別に承認 / 却下
5. 承認された分のみ反映し、AuditLog に記録
```
...
＋ 601, 602 が PMS に追加されています        [ 反映 ] [ 無視 ]
− 208 が PMS から削除されています            [ 反映 ] [ 無視 ]
≠ 305 の客室タイプが異なります
    ProofKeeping: ツイン / PMS: ダブル       [ 反映 ] [ 無視 ]

※ 清掃専用の部屋（B01 パントリー等）は差分に含まれません
```

**MUST**:
- 「削除」の差分を反映する場合、`isActive = false` にするだけで物理削除しない。過去のタスクと証跡が参照しているため。
- `isSellable = false` の部屋を差分対象から除外する。
- 「無視」を選んだ差分は 30 日間再提示しない。
- 差分が 20 件を超える場合、一括承認ボタンを出す。ただし確認ダイアログを挟む。

### 24.5 マスタ変更時のタスクへの影響

**MUST**:
- 客室を `isActive = false` にしても、未完了タスクは自動キャンセルしない。施設責任者に「3件の未完了タスクがあります」と提示し、明示操作を求める。
- 客室タイプを変更しても、既存タスクの `standardMinutes` は変えない。実施時点の条件を保持する。
- 部屋番号の変更は許可するが、`AuditLog` に旧番号を残す。証跡の追跡可能性を保つため。
...
新規タスク:
- P0-21 施設セレクタ（§23）
- P0-22 客室マスタ 方式A（§24.2–24.3）

§24.4 の方式B は P6 で実装する。P0 では以下のみ準備する:
- Room に isSellable / sourceType / externalRoomId カラムを追加
- externalMapping テーブルの定義（使用は P6）

## P0 v1.2 固有の絶対ルール
- 施設サマリーを rollup テーブル以外から取得しない。
- 全社ビューを持たないロールに "ALL" を許可しない（403）。
- 客室の物理削除 API を作らない。isActive = false のみ。
- PMS からの取得データで既存の客室マスタを自動上書きしない。
- 清掃専用の部屋（isSellable = false）を稼働照合の対象にしない。
- 施設切替を AuditLog に記録しない（"ALL" への切替のみ記録）。
```
...
|---|---|---|
| v1.0 | 2026-08-10 | 初版確定 |
| v1.1 | 2026-08-10 | インフラをフルスタック Cloudflare へ変更。§19 シャーディング構成を新設 |
| v1.2 | 2026-08-10 | §23 施設セレクタとマルチ施設 UI を新設。§24 客室マスタの 2 方式を新設。Room に `isSellable` / `sourceType` / `externalRoomId` を追加。タスク P0-21 / P0-22 を追加 |
