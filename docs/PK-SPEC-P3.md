...
2026-08-22 追記（DECISIONS #253）: ①の自己停止

MUST: 上の 2 条件を両方満たしても、**集計キー単位で「値が 0 かつ
bedsUsed > 0」の比率が閾値を超えたら、その集計キーでは①を当てない。**
0 は入力漏れではなく観察とみなし、標本に残したうえで人手の確認へ回す。

  閾値      30%（除外率の警告 15% の 2 倍）
  最小件数  5 件（比率が意味を持たない小さすぎるグループでは自己停止しない）

理由: 上の 3 品目は「宿泊があれば必ず消費される」という前提で選んでいる。
**その前提が誤っていたときに、標本を全部消してしまわないための歯止め。**
①だけで 3 割を超えるなら、前提のほうが誤っている可能性が高い。

歯止めは**除外を止めるだけで、除外を増やすことはない。** 当たらなくなった
0 は標本に残るので、ベースラインは下がる側（＝差異を拾う側）にしか動かない。
見逃しを増やす方向へは倒れない。

自己停止した集計キーは `computeBaseline()` が `zeroReviews` として返し、
週次バッチがログに残す。**除外ではないので baselineExclusionLog には
載せない**（載せる表がまだ無い）。
