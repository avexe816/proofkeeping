...
| `docs/PK-SPEC-P8.md` v1.3 | Workforce と Inventory | GA後3〜6か月 |
...
- **P8 の着手条件（GA 判定の通過）はオーナー指示で解除済み**（2026-08-20 / DECISIONS #219）。P8-01〜P8-09 を GA 判定前に起票・実装してよい。**P7-17 の GA 判定そのものは残る**（解除したのは順序の制約だけ）。給与計算・社会保険・年末調整を作らない原則（PK-SPEC-P8 §1.2）は変わらない。
  - 先行して切り出した支払集計（オーナー指示 2026-08-19）は、引き続き P8 の task ではなく **P5-18**。
- **プラットフォーム運営 12 画面（`ui-prototypes/platform/`）も同じ指示で実装対象**（DECISIONS #219 / #220）。テナントへの課金は**銀行振込**で回し、**Stripe・カード決済・決済代行を実装しない**。
- **2 原則は P8 とプラットフォームにも効く**: **プロトタイプが上限**（無いものを足さない）／**簡素化は必須**（範囲内で不要なものは削る。迷ったら少ない方）。
