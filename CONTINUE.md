...
- main HEAD: `eb0419a docs: P8（Workforce）とプラットフォーム運営 12 画面を起票する (#130)`
- 直近のマージ: **#129**（通知の鈴・ダークモード）→ **#130**（P8 / PF 起票）
- **レビュー待ち**: **#132** P8-01 スタッフ台帳（CI 通過後に自分でマージしてよい）
...
`docs/tasks/P8-02.md`。土台（表・リポジトリ 4 関数・権限・越境テスト・
一覧の在留期限の列・免責文）は #132 で入っている。**残り 3 つ。**

1. **在留資格の編集フォーム**（`recordAudit()` つき）。
   `upsertResidencyRecord()` は用意済み。Zod スキーマを
   `packages/contracts` に足し、`/app/settings/staff` の action に足す。
   **`before` / `after` に個人を特定できる値を残しすぎないこと**（security.md §6）。
2. **90 日前・30 日前の通知**（Cron 07:00 JST）。**60 日は作らない**
   （プロトタイプ ops 07 のトグルが 90 と 30 の 2 つだけ / DECISIONS #221）。
   30 日前は毎日再通知。宛先は `ORG_ADMIN`（INV-08）。
3. **期限切れを新規配分から外す。** `listExpiredResidencyStaffIds()` が
   用意済み。P1-14 の自動配分から呼ぶ。**既存の未完了タスクは残す**
   （現場を止めない / 仕様 §1.4 MUST）。解除は `expiresOn` の更新だけで、
   **手動の解除ボタンを作らない。**
