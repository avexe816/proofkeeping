...
## 最終状態
- main HEAD: P5-07 のマージ後
- 完了: **Phase 0〜4 と P5-01〜P5-07**（P4-08 を除く）
- 次: **P5-08（領収書 PDF と 1 クリック発行）**

## 次にやること
1. `git fetch origin && git checkout main && git pull`
2. `docs/tasks/P5-08.md` を読む（依存 P5-07 は完了済み）
...
### P5-08 に着手するときの注意
- **印紙貼付欄を作らない**（billing.md §3）。「本領収書は電子的に
  発行されたため、収入印紙の貼付を要しません。」を固定表示。
- §8.2 のレイアウト。角印は `sealImageKey`（P5-06 と同じ読み方）。
- 採番は `RCP-{西暦}-{連番4桁}`。`DocumentSequencer` の `RECEIPT`。
- 請求書に紐づかない領収書がありうる（`receipt.invoiceId` は null 可）。
