...
## ⚠ CI が動いていない（2026-08-15 12:15 以降 / **人間の対応が要る**）

**PR #90 はマージ待ち。CI が緑にならないのは、この PR の中身とは無関係。**

`.github/workflows/ci.yml` の全ジョブが **2〜4 秒で failure** になり、
**ログが 404**（生成されていない）。ジョブの `runner_id` は `0`、
`runner_name` は空で、**ランナーが 1 台も割り当てられていない。**

| run | ブランチ | 結果 | 所要 |
|---|---|---|---|
| #196 | main | success | 2 分 46 秒 |
| #199 | claude/continue-z1xuml | success | 3 分 05 秒 |
| **#200** | **main（`01d7c81`）** | **failure** | **6 秒** |
| #201 | claude/continue-z1xuml | failure | 5 秒 |
| **#202** | **claude/continue-86d5jw（PR #90）** | **failure** | **5 秒** |

**#200 は main の HEAD をそのまま回して同じ壊れ方をしている。**
12:15 より前は同じ設定で 3 分かけて緑になっていた。**設定の変更ではなく、
ランナーが確保できていない。** 私有リポジトリなので、**Actions の
利用枠か支払い上限**（Settings → Billing → Actions）をまず疑うこと。

`.claude/rules/workflow.md` §6「CI が赤で原因が自分の変更以外」に該当するため、
**再試行を繰り返さずに止めた。** ローカルでは `pnpm check` が通っている
（**217 ファイル / 5,269 テスト**）。枠が戻ったら PR #90 を再実行して
マージすればよい。
...
- **版 11 以降を実装していない。** 版 10-M で 213 バイト入り、ログイン URL は
  ...
  必ず既知値と突き合わせること。
- **`orgShortId` を印字する**（DECISIONS #185）。§2.4 の図の「施設コード」は
...
- §2.4 の図の「施設コード HTLA」→ **「組織 ID」**（DECISIONS #185）
