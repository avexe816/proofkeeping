...
## 最終状態
- main HEAD: `10ed791`（P7-03 完了）
- 完了: 125 task
- 次: **人間の判断待ち。** P7-02 は OPEN_QUESTIONS #105 が決まるまで着手できない
...
**着手できる実装 task が無い。** 残りは人間の判断か、人間の作業。

1. **OPEN_QUESTIONS #105 を読む。** P7-02 はこれが決まるまで着手できない。
2. 決まったら `docs/tasks/P7-02.md` と `docs/PK-SPEC-P7.md` §2.4 を読む。
...
| task | 誰が | 何をする |
|---|---|---|
| **P7-02 ログイン案内カード PDF** | 人間 | **OPEN_QUESTIONS #105 の判断待ち。** 着手できない |
...
## P7-02 が着手できない理由（OPEN_QUESTIONS #105）

§2.4 の掲示物には**初期 PIN を印字する**と書かれている。ところが、
これを作るには次の 2 つを同時に満たせない。

- **CLAUDE.md §2**「PDF は @react-pdf/renderer（**Queue コンシューマ内のみ**）」
- **DECISIONS #177**「初期 PIN は保存しない。作成の応答で 1 回だけ返す」

Queue へ投げるには PIN をメッセージに載せる必要があり、「保存しない」が崩れる。
リクエストハンドラで作れば #177 は保てるが、CLAUDE.md §2 と §4（CPU 50ms）に反する。

併せて **QR コードを生成する手段が無い。** `packages/pdf` の依存は
`@react-pdf/renderer` と `react` だけで、仕様にも実装方法の指定が無い。

**推奨は「PDF をやめて印刷用 HTML にする」**（#105 の案 a）。
PIN がサーバーの外へ出る経路を増やさずに済み、QR の依存もブラウザ側で閉じる。
現場に貼る紙という用途は、清掃スタッフ向けガイド（P7-15）が既に
印刷用 HTML で満たしている。**§2.4 の版上げが要る。**
...
### 未解決の問い（新しい順）
- #105 ログイン案内カードの PDF が Queue 限定と PIN 非保存に衝突 → **判断待ち**
