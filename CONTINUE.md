...
## 最終状態
- main HEAD: P5-06 のマージ後（aab3f11）
- 完了: **Phase 0〜4 と P5-01〜P5-06**（P4-08 を除く）
- 次: **P5-07（1 クリック発行）★中核機能**
- **⚠ 着手前に人間の判断が要る項目が 2 つある（下記「判断が必要です」）**

## 判断が必要です

### ① 並行セッションとの切り分け（着手前に必ず確認）
main に別セッションの `#48 P5-02・P5-03 取引先と料金の設定画面` が
入っている。こちらの `#45`（API 層）と**同じ task 番号を分担した形**。

P5-07 は §4.1 の 10 手順（採番・スナップショット・ハッシュ・Queue・
トランザクション）が一体で、**分割すると壊れる。** 2 セッションが
同時に着手すると片方の PR がもう片方を上書きする。

**どちらのセッションが P5-07 を持つかを決めてから着手すること。**

### ② §10.6 の「双方が AGREED にしないと発行できない設定」に列が無い
§10.6 は「双方が AGREED にしないと請求書を発行できない**設定が可能**」と
書くが、その設定を持つ列が §2.1 の取引先にも §2.8 にも無い。
一方 §6.1 の本文と `billing_period` の schema 注記は
「**同意していなくても請求はできる**」と書いている。

| 案 | 内容 | 影響 |
|---|---|---|
| A | 常に `AGREED` 必須 | 安全側。ただし合意の口は P5-12 なので、当面 API 経由でしか合意できず請求書を出せない |
| B | 既定は `REVIEWING` からも発行可。設定列は後から足す | §6.1 の本文どおり。ただし §10.6 の「設定」が無いまま出荷される |

**推奨は A。** 理由は 2 つ。
- 状態機械は既にマージ済みで、`ISSUE_INVOICE` は `AGREED` からしか
  通らない（`evaluateBillingPeriodTransition()`）。B にすると、
  出荷済みの判定を緩める向きの変更になる。
- §0.2 の出荷判定に「清掃会社とホテルが同じ明細を見て相違なく合意できる」
  があり、合意を通さない発行を既定にすると出荷判定と噛み合わない。

A を採る場合、**P5-12（合意フロー）を P5-07 より先に回すか**を
合わせて決めること。合意の画面が無いと、現場では請求書が 1 通も出せない。
