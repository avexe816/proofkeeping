...
## 最終状態
- main HEAD: `261f855` の次（P7-12 の PR）
- 完了: 122 task（P7-12 は**ハーネスのみ。実測は人間待ち**）
- 次: **P7-13（セキュリティ再検証）**
...
**P7-13 の完了条件 4 件のうち 2 件は人間が実施。**
- 「全ロール × 全画面 × 全 API のマトリクスを再検証した」→ 自動化できる
  （`permission.spec.ts` の `EXPECTED` を全経路へ広げる形）
- 「Dependabot / gitleaks が動いている」→ **設定ファイルを置ける**が、
  実際に動くのは GitHub 側の設定
- 「宿泊者の個人情報が全テーブルに存在しないことを確認した」→ 自動化できる
  （schema を走査する spec）
...
11. **負荷試験の実測**（P7-12 / §4.2 MUST）。`pnpm loadtest` を
