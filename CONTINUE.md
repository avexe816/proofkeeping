...
## P7-03 に着手する前に（次にやる task）

**仕様**: §2.5。期間 30 日 / 施設 3 つ・客室 150 室まで / 全モジュール利用可 /
終了時は**読み取り専用**へ移行し、データは 90 日保持。
**MUST: トライアル終了でデータを即削除しない。**

いま在るもの・無いものを先に調べてある。

| 要るもの | 現状 |
|---|---|
| `subscription.status = "TRIAL"` / `trialEndsAt` | **列はある**（P0-06） |
| トライアルの上限（3 施設 / 150 室）の判定 | **無い** |
| **読み取り専用モード** | **無い。** `readOnly` を見るコードが 1 行も無い |
| 90 日保持 | **消す仕組みがそもそも無い**ので、現状は「保持されている」 |

### 気をつけること

- **読み取り専用は横断的な変更。** 全部の書き込み経路に効かせる必要がある。
  `AUDITOR`（読取専用ロール）が既に `assertPermission()` で止まっているので、
  **同じ層に寄せられるか**を先に見ること。書き込み経路ごとに `if` を撒くと
  漏れが出る。
- **4xx で塞がないこと**（`degradation/priority.ts` と同じ理由）。
  オフラインキューが `GIVE_UP` にして未送信の記録を捨てる。
- **施設の作成経路が無い**ので、3 施設の上限は当面かけようが無い
  （OPEN_QUESTIONS #103）。**客室 150 室のほうは `createRooms()` に効かせられる。**
- 90 日保持は**何もしないことで満たされる。** 削除する task を書くときに
  この期限を持ち込むこと。
