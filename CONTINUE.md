...
- main HEAD: `9b08dcc P8-01: スタッフ台帳（プロトタイプ ops 07）と P8-02 の土台 (#132)`
- 直近のマージ: **#129**（通知の鈴・ダークモード）→ **#130**（P8 / PF 起票）
- **マージ済み**: **#132** P8-01 スタッフ台帳
- **レビュー待ち**: **#133** P8-02 在留資格の記録（CI 通過後に自分でマージしてよい）
...
`docs/tasks/P8-02.md`。土台（#132）と**記録の口**（#133）は入っている。
**残り 2 つ。**

1. **90 日前・30 日前の通知**（Cron 07:00 JST）。**60 日は作らない**
   （プロトタイプ ops 07 のトグルが 90 と 30 の 2 つだけ / DECISIONS #221）。
   30 日前は毎日再通知。宛先は `ORG_ADMIN`（INV-08）。
2. **期限切れを新規配分から外す。** `listExpiredResidencyStaffIds()` が
