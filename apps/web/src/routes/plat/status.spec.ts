...
describe("逐語の注記（PF-03「逐語で置く注記」）", () => {
  ...
  it("メンテナンスの注記が **03:00〜04:00**（DECISIONS #239 で統一）", () => {
    // プロトタイプの文面は 02:00 だったが、PF-14 の「運用（変更可）」の
    // 既定（03:00）に**注記のほうを合わせた**（オーナー判断）。
    // **境界は 03:00 以上 04:00 未満**なので、それも書いてある。
    expect(ja["plat.status.note.maintenance"]).toBe(
      "清掃業務は早朝から午前中に集中します。深夜帯のメンテナンスウィンドウは 03:00〜04:00 を基本とします（03:00 以上 04:00 未満）。",
    );
    expect(ja["plat.status.note.maintenance"]).not.toContain("02:00");
  });
});
