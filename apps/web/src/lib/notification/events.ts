...
export const NOTIFICATION_EVENTS: readonly NotificationEventDefinition[] = [
  ...
  },
  {
    ...
    // PK-SPEC-P8 §1.4）。
    //
    // **宛先は雇用主の側**（`OWNER` / `ORG_ADMIN`）。INV-08 v2 に合わせて
    // `OWNER` を足した（2026-08-22 / OPEN_QUESTIONS #110 決着 /
    // DECISIONS #261）。**読めるのに期限の知らせが来ない状態を作らない。**
    // 在留期限の更新管理は雇用主の継続的な義務で、知らせが届かないことが
    // そのまま法令上の危うさになる。
    //
    // **人数だけを載せる**（`consumers/residencyAlert.ts` が本文を作る）。
    // 誰の期限が近いかは通知に書かない。
